...
接続したら、Ctrl+Shift+Pでコマンドパレットを開き、`>PlatformIO: New Terminal`を実行すると、PlatformIOのツールへのパスが通ったシェルが開くので、`pio run -e (Environment名) --upload-port (ポート)`を実行する。GUIでのEnvironmentとポートの設定は反映されない。Environmentが1つだけの場合は指定しなくてよい。
