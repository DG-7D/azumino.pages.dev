...
Win10用のアップデートは展開だけならVAIOでなくても実行でき(UACが出るがいきなりインストールはされないので許可してよい)、同様の場所に展開されるのでVAIOの設定だけもらっておく。

![C:\ProgramData\内にVAIOの設定のインストーラを含むファイル群が展開される。インストールする前に「キャンセル」することができる。](vjp131-win10-vcc.png "VJP131のWin10用アップデートの中身")

SFEPは怪しいサイトのSONY版と同一バイナリで、インストーラはなく、普通に適用できた。SONY Shared Libraryはインストール時のチェックはなくそのままインストールできる。VAIOの設定はインストール時にもチェックが入るので回避する必要がある。具体的には、SONY版のVAIOの設定のインストーラを7-zipで展開し、`setup.inx`(機種チェックとかバージョンチェックとかをするスクリプトらしい)を取り出して、VAIOのものを上書きしてから実行する。(`.inx`を逆コンパイルできそうなツールをいくつか見つけたが、どれもうまくいかなかった。)
