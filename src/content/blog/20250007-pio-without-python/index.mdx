...
publishDate: 2025-08-08 20:45:00 +09:00
...
実は、PlatformIO IDEのようなPlatformIO Coreと連携して動作するアプリケーションにおけるCoreのインストール方法も、[公式ドキュメント](https://docs.platformio.org/en/latest/core/installation/integration.html)に記載されている。ここでもPythonが必要とされているが、「ユーザにPythonをインストールするよう要求」のほかに、「ポータブル版Pythonを落としてくる」という選択肢も提示されている。これを手動で行えば、IDEと同じように綺麗にPlatformIO Coreをインストールできるはずである。なお、本当にIDEと同じ状態にするだけなので、すでにPlatformIO IDEを使っている場合はこの手順は飛ばしてよい。
